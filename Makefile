...
build_lambda: $(DOCKERFILE_PATH) $(BUILD_CONTEXT_DIR)
	docker build -f Dockerfile_lambda --tag 302605622298.dkr.ecr.eu-west-1.amazonaws.com/faceanalysis:dev $(BUILD_CONTEXT_DIR)

push_ecr: build_lambda
	aws ecr get-login-password --region eu-west-1 | docker login --username AWS --password-stdin 302605622298.dkr.ecr.eu-west-1.amazonaws.com
	docker push 302605622298.dkr.ecr.eu-west-1.amazonaws.com/faceanalysis:dev

test_lambda: build_lambda
	docker run  -v ~/.aws-lambda-rie:/aws-lambda -p 9000:8080 --entrypoint /aws-lambda/aws-lambda-rie \
    302605622298.dkr.ecr.eu-west-2.amazonaws.com/faceanalysis:dev \
